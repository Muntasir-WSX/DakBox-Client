import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";// আপনার কাস্টম হুক
...
const CashoutRequests = () => {
    ...
    const approveMutation = useMutation({
        ...
        onSuccess: (data) => {
            if (data.success) {
                Swal.fire("Approved!", "Payment has been processed and rider's history updated.", "success");
                queryClient.invalidateQueries(["admin-cashouts"]); // টেবিল রিফ্রেশ করবে
            }
        },
        onError: (error) => {
    ...
    return (
        <div className="p-6">
            <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
                <Banknote className="text-green-600" /> Rider Cashout Requests
            </h2>
                        ...
                        {requests.length === 0 ? (
                            ...
                            requests.map((req, index) => (
                                <tr key={req._id}>
                                    ...
                                    <td>
                                        {req.status === "pending" ? (
                                            <span className="badge badge-warning gap-1 p-3"><Clock size={14}/> Pending</span>
                                        ) : (
                                            <span className="badge badge-success gap-1 p-3 text-white"><CheckCircle size={14}/> Success</span>
                                        )}
                                    </td>
                                    <td>
                                        {req.status === "pending" && (
